...
class STDPSynapseTest(unittest.TestCase):
    ...
    def init_params(self):
        ...
        if use_nestml:
         self.synapse_model = "neuromodulated_stdp_nestml__with_iaf_psc_exp_nestml"
        else:
         self.synapse_model = "stdp_dopamine_synapse"
        self.presynaptic_firing_rate = 0.#20.  # [ms^-1]
        self.postsynaptic_firing_rate = 0.#20.  # [ms^-1]
        self.dopa_rate = 0.#20.
        self.tau_pre = 20.
        self.tau_post = 33.7
        self.init_weight = 35.
        ...
        if use_nestml:
         ...
         self.synapse_parameters["tau_tr_post"] = self.tau_post
         self.synapse_parameters["tau_tr_pre"] = self.tau_pre
         self.synapse_parameters["w"] = self.init_weight
        else:
         self.neuron_parameters = {
            "tau_minus": self.tau_post,
                    }
         self.synapse_parameters[   "tau_plus"]= self.tau_pre
         self.synapse_parameters[    "weight"]= self.init_weight
...
#dopa [ 4.6 46.2 97.5]
    ...
    def do_the_nest_simulation(self):
        ...
        dopa_spike_sender = nest.Create(
            "spike_generator",
            params=(
                {"spike_times": self.hardcoded_dopa_times}))# + self.sim_dura - self.hardcoded_trains_length}))
        dopa_spike_generator = dopa_spike_sender
        ...
        nest.Connect(postsynaptic_generator + post_spike_generator, postsynaptic_neuron,
                     syn_spec={"synapse_model": "static_synapse", "weight": self.J})
        nest.Connect(dopa_spike_generator, parrot_neuron,
                     syn_spec={"synapse_model": "static_synapse", "weight": self.J})
    ...
    def reproduce_weight_drift(self, pre_spikes, post_spikes, dopa_spikes, fname_snip=""):
        ...
        while t < self.sim_dura:
            #print('############################################')
            do_print = True#abs(round(t) - t) < 0.01
            if do_print:
             print("t = " + str(t))
    ...
    def test_stdp_synapse(self):
        ...
        for self.dendritic_delay in [1.]:
            ...
            if use_nestml:
             self.nest_neuron_model = "iaf_psc_exp_nestml__with_neuromodulated_stdp_nestml"
            else:
